...
class TestCase3D(TestCase):
    ...
    def assertAlmostEqualVertices(self, first: Workplane, second: Workplane, places: Optional[int] = None, msg: Optional[str] = None, delta: Optional[float] = None):
        ...
        for f, s in zip(first, second):
            distance = fabs(sqrt((f[0] - s[0]) ** 2 + (f[1] - s[1]) ** 2 + (f[2] - s[2]) ** 2))
            msg = msg or f'{f} != {s} with distance {distance}'
            self.assertAlmostEqual(distance, 0.0, places=places, msg=msg, delta=delta)
