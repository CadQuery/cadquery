...
def test_remove_with_constraints():
    """
    Tests the ability to remove a part from an assembly when the part has constraints.
    """

    # Create an assembly with sample constraints
    assy = cq.Assembly()
    assy.add(box(1, 1, 1), name="part1")
    assy.add(box(2, 2, 2), name="part2", loc=cq.Location(5.0, 5.0, 5.0))
    assy.constrain("part1", "part2", "Point")

    # Remove part2 and make sure the constraint is removed
    assert len(assy.children) == 2
    assert len(assy.constraints) == 1
    assy.remove("part2")
    assert len(assy.children) == 1
    assert len(assy.constraints) == 0

    # Create another assembly with a nested subassembly that has a constraint to the top-level part
    # assy = cq.Assembly()
    # assy.add(box(1, 1, 1), name="part1")
    # subassy = cq.Assembly()
    # subassy.add(box(1, 1, 1), name="part2")
    # subassy.add(box(2, 2, 2), name="part3", loc=cq.Location(2.0, 2.0, 2.0))
    # assy.add(subassy, name="subassy")
    # assy.constrain("part1@faces@>Z", "subassy/part3@faces@<Z", "Axis")
    # assy.constrain("part1@faces@>Z", "subassy/part2@faces@<Z", "Plane")
    # assy.solve()

    # # Make sure that a constraint with a nested subassembly is removed
