...
plate_0 = cq.Workplane("XY").interpPlate(
    edge_points, surface_points, thickness)
...
edge_wire = edge_wire.add(cq.Workplane("YZ").workplane().transformed(offset=cq.Vector(
    0, 0, -7), rotate=cq.Vector(45, 0, 0)).spline([(-7., 0.), (3, -3), (7., 0.)]))
...
fn = 6
thickness = 0.1
edge_points = [[r1*cos(i * pi/fn), r1*sin(i * pi/fn)] if i % 2 ==
               0 else [r2*cos(i * pi/fn), r2*sin(i * pi/fn)] for i in range(2*fn+1)]
edge_wire = cq.Workplane("XY").polyline(edge_points)
r2 = 4.5
surface_points = [[r2*cos(i * pi/fn), r2*sin(i * pi/fn), 1.]
                  for i in range(2*fn)] + [[0., 0., -2.]]
plate_2 = cq.Workplane("XY").interpPlate(edge_wire, surface_points, thickness, combine=True, clean=True, degree=3, nbPtsOnCur=15,
                                         nbIter=2, anisotropy=False, tol2d=0.00001, tol3d=0.0001, tolAng=0.01, tolCurv=0.1, maxDeg=8, maxSegments=49)
...
sa = sin(30. * pi/180.)
# EVEN ROWS
pts = [(-3.0, -3.0), (-1.267949, -3.0), (0.464102, -3.0), (2.196152, -3.0), (-3.0, 0.0), (-1.267949, 0.0), (0.464102, 0.0), (2.196152, 0.0),
       (-2.133974, -1.5), (-0.401923, -1.5), (1.330127, -1.5), (3.062178, -1.5), (-2.133975, 1.5), (-0.401924, 1.5), (1.330127, 1.5), (3.062178, 1.5)]
# Spike surface
thickness = 0.1
fn = 6
edge_points = [[r1*cos(i * 2*pi/fn + 30*pi/180), r1 *
                sin(i * 2*pi/fn + 30*pi/180)] for i in range(fn+1)]
surface_points = [[r1/4*cos(i * 2*pi/fn + 30*pi/180), r1/4*sin(
    i * 2*pi/fn + 30*pi/180), 0.75] for i in range(fn+1)] + [[0, 0, 2]]
edge_wire = cq.Workplane("XY").polyline(edge_points)
plate_3 = cq.Workplane("XY").pushPoints(pts).interpPlate(edge_wire, surface_points, thickness, combine=False, clean=False, degree=2,
                                                         nbPtsOnCur=20, nbIter=2, anisotropy=False, tol2d=0.00001, tol3d=0.0001, tolAng=0.01, tolCurv=0.1, maxDeg=8, maxSegments=9)
...
thickness = 0.1
edge_points = [[[3.54, 3.54], [1.77, 0.0], [3.54, -3.54]], [[-3.54, -3.54], [0.0, -1.77], [3.54, -3.54]], [[-3.54, -3.54], [0.0, -1.77], [3.54, -3.54]],
               [[-3.54, -3.54], [-1.77, 0.0], [-3.54, 3.54]], [[3.54, 3.54], [0.0, 1.77], [-3.54, 3.54]], [[3.54, 3.54], [0.0, 1.77], [-3.54, 3.54]]]
plane_list = ["XZ", "XY", "YZ", "XZ", "YZ", "XY"]
offset_list = [-3.54, 3.54, 3.54, 3.54, -3.54, -3.54]
edge_wire = cq.Workplane(plane_list[0]).workplane(
    offset=-offset_list[0]).spline(edge_points[0])
for i in range(len(edge_points)-1):
    edge_wire = edge_wire.add(cq.Workplane(
        plane_list[i+1]).workplane(offset=-offset_list[i+1]).spline(edge_points[i+1]))
